...
3. Add the `bin` subfolder to your system PATH (for running from shell)
    - cmd:
      ...
      ```plain
      > PATH %PATH%;C:\Users\cajohn\Downloads\Graphviz-14.0.4-win64\bin
      ```

    - PowerShell:

      ```plain
      PS> $env:Path += ';C:\Users\cajohn\Downloads\Graphviz-14.0.4-win64\bin'
      ```

4. Add the `bin` subfolder to VS Code (for integrated terminal):
    - Open `settings.json`
    - Add this:
